	...
	func doUpdate(url string) error {
		...
		if err != nil {
			if e := update.RollbackError(err); e != nil {
				fmt.Println("Failed to rollback from bad update: %v", e)
			}
		}
		return err
	}
...
have an appropriate checksum (that was otherwise retried via a secure channel)
...
In order to update a Go application with go-update, you must distribute it as a single executable.
